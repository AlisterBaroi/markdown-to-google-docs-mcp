...
export default function McpPanel({ user, mcpToken, onRegenerateToken, onBack }: McpPanelProps) {
  ...
  const osLabel = (platform: string) => {
    switch (platform) {
      ...
      default: return platform || "Unknown OS";
    }
  };
  ...
  // Tick once per second so uptime counts up between agent polls
  useEffect(() => {
    const timer = setInterval(() => setNow(Date.now()), 1000);
    return () => clearInterval(timer);
  }, []);

  // Poll the backend for agents currently connected via SSE on this token
  useEffect(() => {
    if (!mcpToken) return;
    let cancelled = false;
    ...
    const fetchAgents = async () => {
      try {
        const res = await fetch(`/api/mcp/agents?token=${encodeURIComponent(mcpToken)}`);
        if (!res.ok) return;
        const data = await res.json();
        if (!cancelled) setAgents(Array.isArray(data.agents) ? data.agents : []);
      } catch {
        /* transient network error — keep last known list */
      }
    };
                ...
                {agents.map((agent) => (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2 mb-[0.275625rem]">
                        <span className="text-[0.9375rem] font-semibold text-slate-700 dark:text-slate-300 truncate ml-1.5">
                          {agent.name}
                          {agent.version && (
                            <span className="text-slate-400 dark:text-slate-500 font-normal"> v{agent.version}</span>
                          )}
                        </span>
                        <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse shrink-0 mr-1.5"></div>
                      </div>
