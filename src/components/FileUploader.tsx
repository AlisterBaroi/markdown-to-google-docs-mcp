...
export default function FileUploader({ onFilesParsed, uploadedFiles, onRemoveFile, onClearAll }: FileUploaderProps) {
  ...
  return (
    <div className="bg-white border border-slate-200 p-6 rounded-2xl flex flex-col gap-5 shadow-sm" id="file-uploader-panel">
      {/* Drop Zone */}
      <div
      ...
      >
        ...
        <h3 className="font-semibold text-slate-800 text-base">Upload Markdown Files</h3>
        <p className="text-sm text-slate-500 mt-1">Drag & drop your .md files here, or click to browse</p>
      </div>

      {error && (
        <div className="flex items-start gap-2 p-3 bg-amber-50 rounded-xl border border-amber-100 text-amber-700 text-xs font-semibold" id="upload-error-indicator">
      ...
      {uploadedFiles.length > 0 && (
        <div className="flex flex-col gap-2.5 mt-1" id="uploaded-files-queue">
          <div className="flex items-center justify-between text-xs px-1">
            ...
            <button
              onClick={onClearAll}
              className="text-red-500 hover:text-red-700 font-bold transition cursor-pointer select-none"
          ...
          <div className="max-h-[220px] overflow-y-auto flex flex-col gap-2 pr-0.5">
            {uploadedFiles.map((file, idx) => (
              <div
                key={`${file.name}-${idx}`}
                className="flex items-center justify-between p-2.5 rounded-lg border border-slate-100 bg-slate-50 hover:bg-slate-150/40 transition"
                id={`queue-item-${idx}`}
              >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {file.status === 'converting' && (
                    <span className="text-[10px] bg-yellow-50 text-yellow-600 border border-yellow-100 px-2 py-0.5 rounded-full font-bold animate-pulse">
                      Converting...
                    </span>
                  )}
                  {file.status === 'success' && (
                  ...
                  {file.status === 'failed' && (
                    <span className="text-[10px] bg-red-50 text-red-600 border border-red-100 px-2 py-0.5 rounded-full font-bold" title={file.error}>
                      Failed
                    </span>
                  )}
                  ...
                  <button
                    ...
                    className="p-1 hover:bg-gray-200/60 text-gray-400 hover:text-gray-600 rounded-lg transition"
                    title="Remove file"
